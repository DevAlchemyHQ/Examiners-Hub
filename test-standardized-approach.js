...
// Test payload for BULK DEFECTS mode (using unified format)
...
// Test payload for SELECTED IMAGES mode (using unified format)
...
async function testStandardizedApproach() {
  console.log('🧪 TESTING STANDARDIZED APPROACH');
  console.log('=====================================\n');

  // Test BULK DEFECTS mode
  console.log('📦 TESTING BULK DEFECTS MODE (Standardized)...');
  try {
    ...
    if (bulkResponseData.statusCode === 200) {
      ...
      if (bulkDownloadResponse.ok) {
        ...
        const bulkFilename = `standardized-bulk-test-${Date.now()}.zip`;
  ...
  console.log('🖼️ TESTING SELECTED IMAGES MODE (Standardized)...');
  try {
    ...
    if (imagesResponseData.statusCode === 200) {
      ...
      if (imagesDownloadResponse.ok) {
        ...
        // Save the file for inspection
        const fs = await import('fs');
        const imagesFilename = `standardized-images-test-${Date.now()}.zip`;
  ...
  console.log('\n=====================================');
  console.log('🏁 STANDARDIZED APPROACH TESTING COMPLETE');
