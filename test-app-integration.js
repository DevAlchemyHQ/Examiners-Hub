...
// Simulate the actual application data structure
function simulateAppData() {
  // Simulate selected images mode data
  ...
  // Simulate bulk defects mode data
  const bulkDefects = [
    {
      id: "bulk-1",
      photoNumber: "1",
...
function transformSelectedImagesForLambda(selectedImages, formData) {
  console.log('🔄 APP: Transforming selected images for Lambda...');
  ...
  const unifiedImages = selectedImages.map((image, index) => {
    ...
    if (!s3Key && image.publicUrl) {
      ...
      console.log('APP: Extracted S3 key from publicUrl:', s3Key);
    }
    
    if (!s3Key) {
      const userId = image.userId || 'anonymous';
      s3Key = `users/${userId}/images/${image.s3Key || image.file?.name || 'unknown'}`;
      console.log('APP: Constructed S3 key fallback:', s3Key);
    } else {
      if (!s3Key.startsWith('users/')) {
        const userId = image.userId || 'anonymous';
        s3Key = `users/${userId}/images/${s3Key}`;
        console.log('APP: Constructed full S3 key from partial:', s3Key);
      }
    }
...
function transformBulkDefectsForLambda(bulkDefects, images, formData) {
  console.log('🔄 APP: Transforming bulk defects for Lambda...');
  ...
  const unifiedImages = defectsWithImages.map(defect => {
    ...
    if (correspondingImage.s3Key) {
      ...
      console.log('APP: Using stored s3Key:', s3Key);
      ...
      if (correspondingImage.id) {
        if (correspondingImage.id.startsWith('local-')) {
          ...
          console.log('APP: Constructed S3 key from local image ID:', s3Key);
        } else if (correspondingImage.id.startsWith('s3-')) {
          s3Key = `users/${userId}/images/${defect.selectedFile}`;
          console.log('APP: Constructed S3 key for S3-loaded image:', s3Key);
        } else {
          s3Key = `users/${userId}/images/${correspondingImage.id}-${defect.selectedFile}`;
          console.log('APP: Constructed S3 key from ID fallback:', s3Key);
        }
      } else {
        s3Key = `users/${userId}/images/${defect.selectedFile}`;
        console.log('APP: Final fallback S3 key:', s3Key);
      }
    }
...
async function testAppIntegration() {
  ...
  const { selectedImages, bulkDefects, formData } = simulateAppData();

  // Test 1: Selected Images Mode
  console.log('📋 TEST 1: Selected Images Mode (App Integration)...');
  try {
    ...
    if (responseData.statusCode === 200) {
      ...
      if (downloadResponse.ok) {
        ...
        const filename = `app-integration-selected-${Date.now()}.zip`;
        ...
        try {
          ...
          if (imageFiles.length > 0) {
            ...
            console.log(`❌ No images in ZIP - Lambda may need investigation`);
          }
        } catch (zipError) {
  ...
  // Test 2: Bulk Defects Mode
  console.log('📋 TEST 2: Bulk Defects Mode (App Integration)...');
  try {
    ...
    if (responseData.statusCode === 200) {
      ...
      if (downloadResponse.ok) {
        ...
        // Save and check contents
        const fs = await import('fs');
        const filename = `app-integration-bulk-${Date.now()}.zip`;
        fs.writeFileSync(filename, Buffer.from(buffer));
