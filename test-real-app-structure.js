...
// Test payload that mimics the real application structure
const realAppPayload = {
  selectedImages: [
    {
      id: "s3-1753036987102",
...
async function testRealAppStructure() {
  try {
    console.log('🧪 Testing with real application data structure...');
    console.log('Payload:', JSON.stringify(realAppPayload, null, 2));
    
    const command = new InvokeCommand({
      FunctionName: 'download-generator',
      InvocationType: 'RequestResponse',
      Payload: JSON.stringify(realAppPayload)
    });
    ...
    if (responseData.statusCode === 200) {
      ...
      if (downloadResponse.ok) {
        ...
        const filename = `real-app-test-${Date.now()}.zip`;
        ...
        try {
          ...
          if (imageFiles.length === 0) {
            console.log(`❌ NO IMAGES FOUND IN ZIP! This confirms the issue.`);
          } else {
            console.log(`✅ Images found in ZIP - Lambda is working correctly.`);
          }
