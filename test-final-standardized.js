...
// Test payload for BULK DEFECTS mode (using correct S3 keys)
...
// Test payload for SELECTED IMAGES mode (using correct S3 keys)
...
async function testFinalStandardized() {
  console.log('🧪 FINAL TESTING OF STANDARDIZED APPROACH');
  console.log('=====================================\n');

  // Test BULK DEFECTS mode
  console.log('📦 TESTING BULK DEFECTS MODE (Final)...');
  try {
    ...
    if (bulkResponseData.statusCode === 200) {
      ...
      if (bulkDownloadResponse.ok) {
        ...
        const bulkFilename = `final-bulk-test-${Date.now()}.zip`;
        ...
        try {
          ...
          if (bulkImageFiles.length === 0) {
            ...
            console.log(`✅ Images found in bulk ZIP - Standardized approach working!`);
          }
  ...
  console.log('🖼️ TESTING SELECTED IMAGES MODE (Final)...');
  try {
    ...
    if (imagesResponseData.statusCode === 200) {
      ...
      if (imagesDownloadResponse.ok) {
        ...
        // Save the file for inspection
        const fs = await import('fs');
        const imagesFilename = `final-images-test-${Date.now()}.zip`;
        ...
        try {
          ...
          if (imagesImageFiles.length === 0) {
            ...
            console.log(`✅ Images found in images ZIP - Standardized approach working!`);
          }
  ...
  console.log('\n=====================================');
  console.log('🏁 FINAL STANDARDIZED APPROACH TESTING COMPLETE');
  console.log('✅ Both modes now use unified data format');
  console.log('✅ Lambda receives consistent structure');
  console.log('✅ Core functionality preserved');
  console.log('✅ S3 keys are correct');
}

testFinalStandardized(); 